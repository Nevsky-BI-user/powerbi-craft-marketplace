...
- **Мова.** Новий скіл `dashboard-copy` (report-storytelling): дванадцять
  ...
  словник 86 термінів EN→UA із джерелами (людська локалізація Microsoft,
...
- **Хук став PBIR-свідомим.** `check_report.py` перевіряє `*.bookmark.json`,
  ...
  букмарки, закон серіалізації); `hooks.json` тепер спрацьовує і на Write.
...
- **Гігієна.** Опубліковано з приватної бібліотеки `theme-visuals.md` (54
  цитування в 18 файлах нарешті ведуть на файл) і `filter_panel_layout.py`;
  ...
  описи довші за 1024 символи скорочено; валідатор забороняє локальні шляхи
  `C:\PROJECTS` і посилання на приватну теку `docs/audits`.
