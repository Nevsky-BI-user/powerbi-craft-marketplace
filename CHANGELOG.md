...
Доменна бізнес-термінологія: сім словників для дашбордів, звітів і застосунків
CRM та сервісу, разом понад 800 термінів із одиницями.

- `dashboard-copy` отримав теку `references/domains/` — по одному файлу на домен,
  у кожному понад сто термінів у форматі «EN → підпис українською → примітка про
  одиницю, шкалу чи пастку», а наприкінці таблиця кальок «не пиши / пиши»:
  - **продажі й CRM** (151 термін): сутності, етапи угоди, категорії прогнозу,
    показники портфеля й утримання, кнопки та повідомлення застосунку;
  - **маркетинг** (109): канали, кампанії, атрибуція, вартість залучення,
    розсилки, A/B-тести, приклади заголовків із базою порівняння;
  - **фінанси** (122): статті звітності за НП(С)БО 1, бюджет і план-факт,
    рентабельність, ліквідність, грошові потоки, дебіторська заборгованість;
  - **HR** (118): чисельність за Держстатом, рух персоналу, підбір, оплата
    праці, оцінювання, залученість, робочий час;
  - **роздріб і електронна комерція** (114): товарна ієрархія, кошик, ціни й
    акції, онлайн-вітрина, запаси, доставка, повернення;
  - **логістика, постачання і виробництво** (109): закупівлі, склад, транспорт,
    випуск, якість, обладнання, охорона праці;
  - **сервіс і підтримка** (115): звернення, черги, статуси, рівень
    обслуговування, задоволеність, показники контакт-центру.
- Опис `dashboard-copy` тепер називає застосунки CRM і сервісу та етапи зі
  статусами — раніше ці запити не мали власника; додано порядок пошуку терміна
  й пункт перевірки «термін узятий з доменного файла».
- `ukrainian-ui-copy`, `data-storytelling` і `kpi-data-entry-app` вказують на
  доменні словники: вибір слова — там, а тут відмінок, твердження й механіка.
- Версії: report-storytelling 0.2.2, project-bootstrap 0.1.8.

## 0.1.21 — 2026-09-02

Уроки з аналізу сесій серпня–вересня: у чотири скіли дописано те, що ламалося
насправді, з правилом на кожен випадок.

- `claude-md-bootstrap` — розділ про хуки й огорожі: поле `if` звужує хук до
  одного інструмента, попередження з `exit 0` модель не бачить, обходи
  регулярок (`commit -am`, `git add ./`), межі хука за cwd, `git add` лише
  явними шляхами, подвійний захист команд деплою, мертві посилання на
  глобальні скіли в хмарних сесіях, ротація «Поточного стану», суперечність
  правила з кодом.
- `model-orchestration` — шість правил із живих сесій: незалежна перевірка
  перед PR (57 знахідок скептика на чотирьох «готових» пакетах), режим здачі
  до фан-ауту, факти оточення в брифі, контракт приватності, поведінка при
  блокуванні класифікатором, точкові правки згенерованих файлів.
- `dashboard-copy` — розділ «Що ламалося насправді»: один підпис на двох
  шкалах, назва осі як розсинхронна копія, опис графіка як alt-текст; два
  нові пункти перевірки.
- `rayfin-bootstrap` — ціль деплою: без `rayfin/.deployments.json` `rayfin up`
  створює дублікат у «My Workspace», dry-run цього не показує, хмарна сесія
  деплоїти не може; подвійний захист deny + хук.
- Версії: project-bootstrap 0.1.7, agent-craft 0.2.2, report-storytelling 0.2.1.

## 0.1.20 — 2026-09-02

Великий випуск за трьома побажаннями: ділова українська для дашбордів,
PBIR-навігація, букмарки й іконки, а також гігієна всієї бібліотеки.

- **Мова.** Новий скіл `dashboard-copy` (report-storytelling): дванадцять
  правил ділової української для підписів, довідок і перекладу термінів,
  словник 115 термінів EN→UA із джерелами (людська локалізація Microsoft,
  НП(С)БО 1, Держстат, НБУ) і довідник форматів чисел та дат за CLDR uk.
  `ukrainian-ui-copy` отримав правила нерозривного пробілу, санкціонований
  форматер і CLDR-скорочення місяців; `data-storytelling` — лексикон баз
  порівняння (план-факт, з початку року, наростаючим підсумком, темп
  зростання ≠ темп приросту); `plain-report` — ділові кальки, правопис-2019 та
  українські джерела простої мови. Скорочення «п.п.» замінено на «в. п.» —
  так пишуть НБУ і словники.
- **Букмарки під обидва формати.** `powerbi-bookmarks` тепер описує PBIR
  enhanced першим (файли `definition/bookmarks`, чотири перемикачі `options`,
  дві форми `bookmarks.json`, групи для навігаторів) із Legacy як додатком,
  має українські тригери й скрипт `pbir_schema_validate.py` — перевірку за
  схемою плюс перехресні перевірки посилань.
- **Кнопки й навігатори.** Новий скіл `pbi-buttons-actions`: усі типи дій
  кнопки з обов'язковими ключами, п'ять станів, іконка з підписом, закон
  серіалізації — на перевірених JSON із живих звітів. У
  `pbi-navigation-tabs` — канон `pageNavigator` і `bookmarkNavigator` та їхні
  обмеження.
- **Меню на всіх сторінках.** `pbi-navigation-variants` отримав
  `scripts/build_nav.py`: за тим самим конфігом, що й прев'ю, будує обране
  меню на кожній видимій сторінці (V1, V2, V3, V6, V7), адаптує геометрію до
  розміру канви, виділяє поточну сторінку, повертає звіт покриття; ідемпотентний.
  Довідник із таблицею відповідності «застосунки Rayfin → механізми PBIR».
- **Іконки.** `icon-set-manager` має політику «де іконка потрібна, а де —
  шум», типовий набір для навігації і два шляхи доставки.
- **Хук став PBIR-свідомим.** `check_report.py` перевіряє `*.bookmark.json`,
  `bookmarks.json` і `visual.json` (обсяг букмарки проти `targetVisualNames`,
  форми елементів індексу, резолвінг дій кнопок у реальні сторінки й
  букмарки, закон серіалізації); попередження теж повертаються Claude
  (exit 2 — інакше stderr хука не видно); крива структура дає пояснення, а не
  трейсбек; `hooks.json` тепер спрацьовує і на Write.
  Фікстури `scripts/fixtures/pbir-min` та `pbir-broken` для тестів.
- **Розгортання.** `pbip-deploy` описує report-only PBIR-деплой, вимогу
  `byConnection` для REST, застереження про RegisteredResources і PNG-перевірку
  кожної сторінки після змін навігації; `fabric-cli-powerbi` — українські
  тригери й межі.
- **Рев'юери** `pbip-pr-reviewer` і `pbi-report-review` бачать
  `definition/bookmarks/**` і кнопки.
- **Гігієна.** Опубліковано з приватної бібліотеки `theme-visuals.md` (51
  цитування в 16 файлах нарешті ведуть на файл) і `filter_panel_layout.py`;
  13 описів без меж «Do NOT» отримали їх; розведено однакові тригерні фрази
  (типографіка, теплова карта, панель фільтрів, меню звіту); мертві посилання
  на зовнішні скіли позначено як зовнішні залежності (розділ у README);
  описи довші за 1024 символи скорочено, і валідатор відтепер це перевіряє;
  валідатор забороняє локальні шляхи `C:\PROJECTS`, посилання на приватну
  теку `docs/audits` і внутрішні назви звітів.
- Версії: pbi-report-ux 0.2.0, report-storytelling 0.2.0, pbi-design-language
  0.1.5, pbi-visuals 0.1.4, pbip-devops 0.1.4, project-bootstrap 0.1.6,
  azure-ops 0.2.6, dax-craft 0.2.1, pbi-quality 0.1.3, agent-craft 0.2.1.
