...
| Performance-bar / needle status | `ramp/rag` (`pbi-design-system` §1.3) or `good`/`bad` |
...
| Status/target-zone color as arbitrary hex | Theme drift | `ramp/rag` or `good`/`bad` from `pbi-design-system` |
