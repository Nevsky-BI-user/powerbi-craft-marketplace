...
description: Use when building or polishing a React SPA (дешборд, звіт, форма внесення) - tooltips, side drawers, sticky table headers, chart entrance animations, optimistic saves, route code splitting, error boundaries, URL state, count-up numbers, skeletons - or when hitting symptoms like sticky th не липне в overflow-x-auto, підказку обрізає overflow, підказки дублюють видимі підписи, анімації «зникли» бо reveal розкрив усю сітку одразу, цифри застигли на 0 у фоновій вкладці. Triggers - "живі підказки", "дровер", "липка шапка", "анімації графіків", "оптимістичне збереження", "React.lazy", "скелетон", "місяць в URL". Do NOT trigger for Power BI report navigation, tooltips or bookmarks (pbi-navigation-tabs, pbi-tooltips, powerbi-bookmarks), Rayfin platform scaffolding or deploy (rayfin-bootstrap), the data-entry app blueprint (kpi-data-entry-app).
---
...
| «шапка таблиці в PDF не дублюється» при живому `table-header-group` | Chromium не малює в ПОВТОРЕНІЙ шапці вміст форм-контролів: смуга й межа на сторінках 2+ є, а `th` із `<button>` сортування — порожні (текст, `<span>`, `<a>`, absolute-нащадки повторюються нормально; від display кнопки не залежить) → `@media print { thead button { display: contents } }`, а глиф «↕» під `print:hidden` |
| «фікс друку не працює», хоч у репро працює | синтетичне репро структури не ловить дефект, бо той сидить у вмісті комірки → друкувати ЖИВУ сторінку: `Page.printToPDF` (`transferMode:'ReturnAsStream'` + `IO.read`) і шукати підпис шапки посторінково в тексті PDF; текст комірки Chrome може розривати `U+FFFE`, тож звіряти нормалізовано |
| пів-аркуша порожні в PDF, документ на третину довший | суцільне `break-inside: avoid` на тип елемента (`section`, `.card`): блок, вищий за залишок аркуша, Chromium переносить ЦІЛКОМ → берегти лише атомарне (діаграма з div-ів, плитка, рядок) точковим класом `.print-keep`, а шапку блока чіпляти до вмісту `break-after: avoid`; міряти заповненість аркуша найнижчим не-білим пікселем, а не оком |
