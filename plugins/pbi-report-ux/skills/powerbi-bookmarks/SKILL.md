...
Editing bookmarks by hand in `report.json` is fragile. This skill encodes the mental model and the **one gotcha that breaks everything**, plus a byte-faithful editing harness.
...
- A group's own default lives in its container config: `singleVisualGroup.isHidden` (usually `true`). The active bookmark overrides it — but only if the group is a target (see gotcha).

### Captured filters inside bookmarks (shape differs from live!)
...
## Tab-isolation pattern (one group per tab)
...
## Placeholders ("В розробці") and z-order
...
## Byte-faithful editing — use `pbir.py` (in this skill folder)
...
| Moved/resized a visual, committed, **canvas unchanged** | wrote only the container mirror (`vc['x']…`) and not `cfg['layouts'][0]['position']` — see below |
| Edit doesn't appear in Desktop | Desktop was open during edit — close WITHOUT saving, reopen |
| Hidden filter works on page load, gone after clicking a tab | bookmarks that target the visual replay a captured card with no `filter` body (or no card) — patch every such capture, see "A new filter on a visual dies on the first tab click" |
...
3b. If the change is a **filter** on a visual, patch the captured card in every bookmark that already targets that visual (see the filter gotcha) — a clean live diff does not mean the filter survives a click.
4. Validate: JSON reloads, diff localized, run the diagnostic snippet (all touched ids `inTarget=True`).
...
## ⚠️ Geometry is stored TWICE — write both, verify the right one
...
## Visual properties: where they REALLY live (linter strips wrong placements)
